import flet as ft
import traceback # Serve per catturare gli errori

# --- CONFIGURAZIONE ---
# NOTA: Manteniamo lo slash "/" perché assets_dir è impostato su "assets"
BOOKS_DATA = {
...
def main(page: ft.Page):
    ...
    try:
        ...
        audio_player = None
        try:
            # Troviamo l'audio ma NON lo carichiamo subito in autoplay per non bloccare
        ...
        def get_stored_data():
            try:
                ...
                if "C:" in saved_pic or "\\" in saved_pic: saved_pic = "/user.svg"
                ...
                # Se la memoria fallisce, usa dati default
    ...
    # --- CATTURA DEGLI ERRORI ---
        ...
        # Se qualcosa crasha, mostriamo l'errore sullo schermo invece che il bianco
        page.clean()
        page.bgcolor = "white"
        page.add(
            ft.Column([
                ft.Icon(ft.icons.ERROR, color="red", size=50),
                ft.Text("OPS! QUALCOSA È ANDATO STORTO", size=20, weight="bold", color="red"),
                ft.Text(f"Errore: {str(e)}", color="black"),
                ft.Text("Dettagli Tecnici:", weight="bold", color="black"),
                ft.Text(traceback.format_exc(), size=10, color="grey", font_family="monospace")
