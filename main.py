...
# --- VERSIONE 30.0: FIX ARGOMENTI POSIZIONALI ---
# Questa versione rimuove "name=" e usa gli argomenti diretti per evitare il TypeError
...
(Testo completo nell'app finale...)
"""
...
def main(page: ft.Page):
    ...
    def show_test_message(e):
        page.snack_bar = ft.SnackBar(content=ft.Text("Funzione in arrivo!"))
    ...
    # FIX: Rimosso argomenti opzionali in eccesso, usiamo Container semplice
    header_logo = ft.Container(
        width=65, height=65, border_radius=18, alignment=ft.Alignment(0, 0), 
        content=ft.Text("M2G", color="white", size=22, weight="w300")
    )
    header_container = ft.Container(
        padding=ft.padding.only(top=20, bottom=20, left=20, right=20), 
        content=ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=15, controls=[header_logo, txt_welcome_name])
    )

    # User Page - FIX: ft.Icon("person") SENZA name=
    img_profile_view = ft.Icon("person", size=100, color=get_c("primary"))
    ...
    btn_upload_photo = ft.ElevatedButton("CARICAMENTO FOTO (Presto disponibile)", color="white", disabled=True)
